coef.mglasso <- function(object, ..., nrho = 1L, type = c("theta", "sigma", "mu"), print.info = FALSE, digits = 3L){
    ...
    out <- switch(type,
                    "mu" = list(object$mu[, nrho, drop = FALSE], "Expected values"),
    ...
    if(print.info) {
        ...
        if(ncomp == 1) {
            ...
            for(k in 1:ncomp) {
                ...
                id2 <- Ck[(p_k1 + 1):p]
                cat(msg, k, ")\n\n", sep = "")
                if(type == "mu") print.table(coef.mat[sort(id1), , drop = FALSE], digits = digits, zero.print = ".", ...)
                else print.table(coef.mat[sort(id1), sort(id1), drop = FALSE], digits = digits, zero.print = ".", ...)
                cat("\n")
                Ck[1:p_k2] <- id2
                Ck[(p_k2 + 1):p] <- id1
